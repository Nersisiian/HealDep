...
pub fn run(project_dir: &str) -> Result<()> {
    ...
    if !config_path.exists() {
        ...
        println!("{}", style("✅ Создан healdep.toml с базовой конфигурацией AI.").green());
    } else {
    ...
    if !action_path.exists() {
        fs::write(&action_path, include_str!("../../../.github/workflows/healdep-action.yml"))?;
        println!("{}", style("✅ Добавлен GitHub Action в .github/workflows/healdep-action.yml").green());
    }

    // Предлагаем вставить бейдж в README
    let readme_path = dir.join("README.md");
    if readme_path.exists() {
        println!("{}", style("💡 Добавьте этот бейдж в ваш README.md:").yellow());
        println!("{}", BADGE_URL);
    } else {
        println!("{}", style("💡 Создайте README.md с этим бейджем:").yellow());
        println!("{}", BADGE_URL);
    }
    Ok(())
}
