...
async fn generate_ollama(prompt: &str, config: &AiConfig) -> anyhow::Result<String> {
    let url = config.ollama_url.as_deref().unwrap_or("http://localhost:11434/api/generate");
...
async fn generate_openai(prompt: &str, config: &AiConfig) -> anyhow::Result<String> {
    let api_key = config.openai_api_key.as_ref().ok_or(anyhow::anyhow!("Не указан openai_api_key"))?;
...
impl AiGenerator for AiConfig {
    fn generate(&self, prompt: String) -> Pin<Box<dyn Future<Output = anyhow::Result<String>> + Send + '_>> {
